...
namespace KopliSoft.Inventory
{
    public class DragItem : MonoBehaviour, IDragHandler, IPointerDownHandler, IEndDragHandler
    {
        ...
        void Start()
        {
            ...
            characterSwitch = GameObject.FindGameObjectWithTag("Characters").GetComponent<CharacterSwitch>();
        }
        ...
        private void DropItem()
        {
            ...
            PickUpItem pickup = dropItem.GetComponent<PickUpItem>();
            pickup.item = this.gameObject.GetComponent<ItemOnObject>().item;
            pickup.SetInteractorLayer(new string[]{"Player"});
            pickup.SetInteractableID(3);

            dropItem.transform.position = FindDropPosition();
            inventory.OnUpdateItemList();
        ...
        {
            Transform drop = characterSwitch.GetCurrentCharacter().transform.Find("Drop");
            if (drop == null)
            {
                drop = characterSwitch.GetCurrentCharacter().transform;
            }
