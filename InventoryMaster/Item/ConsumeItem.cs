...
namespace KopliSoft.Inventory
{
    public class ConsumeItem : MonoBehaviour, IPointerDownHandler
    {
        ...
        private CraftSystem playerCraftSystem;

        void Start()
        {
            inventoryInstance = GameObject.FindWithTag("InventoryController");
            playerInventory = inventoryInstance.GetComponent<InventoryController>();
            ...
            playerMainInventory = playerInventory.inventory.GetComponent<Inventory>();
            playerCraftSystem = playerInventory.craftSystem.GetComponent<CraftSystem>();
        }
        ...
        public void OnPointerDown(PointerEventData data)
        {
            if (this.gameObject.transform.parent.parent.parent.GetComponent<EquipmentSystem>() == null)
            {
                ...
                if (data.button == PointerEventData.InputButton.Right)
                {
                    ...
                    {
                        bool check = playerMainInventory.checkIfItemAllreadyExist(item.itemID, item.itemValue);

                        if (!check)
                        {
                            playerMainInventory.addItemToInventory(item.itemID, item.itemValue);
                            playerMainInventory.stackableSettings();
                        }
                        playerCraftSystem.deleteItems(item);
                                    ...
                                    if (itemTypeOfSlot[i].Equals(item.itemType))
                                    {
                                        if (eS.transform.GetChild(1).GetChild(i).childCount != 0)
                                        {
                                            ...
                                            if (this == null)
                                            {
                                                ...
                                                dropItem.transform.localPosition = inventoryInstance.transform.localPosition;
                    ...
                    //Characters can only consume their own items
                    if (inventory != null && inventory.CompareTag("MainInventory"))
                    {
                        doConsume(gearable, inventory);
                    }
        ...
        private void doConsume(bool gearable, Inventory inventory)
        {
            if (!gearable && item.itemType == ItemType.Consumable)
            {
